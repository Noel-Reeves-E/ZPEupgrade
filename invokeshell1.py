...
# ============================================================
#  run_cmd  --  for single commands, uses exec_command
#  From testing: exec_command only works with ONE command at a time.
#  Multi-step toolkit commands (backup, upgrade) use shell_cmd instead.
# ============================================================
...
# ============================================================
#  shell_cmd  --  for multi-step Nodegrid toolkit commands
#  Uses invoke_shell so we can send multiple lines interactively.
#  Used only for save_settings (backup) and software_upgrade.
# ============================================================

def shell_cmd(ssh, commands, wait=8):
    # commands is a list of strings, each sent as a separate line
    shell = ssh.invoke_shell(width=200, height=50)
    time.sleep(2)
    # drain the login banner
    while shell.recv_ready():
        shell.recv(4096)
    # send each command line one by one
    for cmd in commands:
        shell.send(cmd + "\n")
        time.sleep(1)
    # wait for the toolkit to finish processing
    time.sleep(wait)
    output = ""
    while shell.recv_ready():
...
def upgrade_one_device(ip, num, total):
    ...
    try:
        ...
        # Single command -- exec_command works fine here
        # ----------------------------------------------------------
        print(f"\n  [Pre-Check] Capturing console device status (show access/)...")
        pre_access_out = run_cmd(ssh, "show access/", timeout=30)
        print(f"    Captured OK")

        # ----------------------------------------------------------
        # STEP 2: Pre-upgrade backup
        # Multi-step toolkit command -- must use shell_cmd (invoke_shell)
        # because exec_command only runs one command at a time
        # ----------------------------------------------------------
        ...
        # Uses SFTP -- completely separate from CLI, not affected by the above
        # ----------------------------------------------------------
        ...
        # Both are single commands -- exec_command works fine
        # ----------------------------------------------------------
        ...
        try:
            ...
            # wait=5 is short on purpose -- device starts rebooting, we don't need to wait for output
        except Exception:
        ...
        # ----------------------------------------------------------
        # POST-UPGRADE: Validate -- ssh already reconnected in the loop above
        # All single commands -- exec_command works fine
        # show system/about/ gives us hostname (system:), version (software:), uptime (uptime:)
        # ----------------------------------------------------------
        print(f"\n  [Post] Running post-upgrade validation...")
        try:
            about_out = run_cmd(ssh, "show system/about/", timeout=30)
            hostname = ""
            version  = ""
            uptime   = ""
            ...
            # Console status AFTER -- compare with pre-upgrade capture
            post_access_out = run_cmd(ssh, "show access/", timeout=30)
            pre_sorted  = sorted(pre_access_out.splitlines())
            post_sorted = sorted(post_access_out.splitlines())
            console_match = "YES" if pre_sorted == post_sorted else "NO"
