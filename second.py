...
POLL_EVERY       = 15    # seconds between each probe attempt
MAX_WAIT         = 1800  # give up after 30 minutes total (covers 10min pending + reboot)
...
# ============================================================
#  RUN A SINGLE COMMAND  --  returns output string
#  Uses exec_command so each call is one clean independent shot
# ============================================================
...
def upgrade_one_device(ip, num, total):
    ...
    # Create one SSH object -- reused for the entire device lifecycle
    ...
    try:
        ...
        # All save_settings sub-commands sent as one heredoc block
        # ----------------------------------------------------------
        ...
        backup_cmd = (
            f"save_settings <<EOF\n"
            f"set destination=remote_server\n"
            f"set url={BACKUP_SERVER_URL}/{backup_file}\n"
            f"set username={BACKUP_SERVER_USER}\n"
            f"set password={BACKUP_SERVER_PASS}\n"
            f"save\n"
            f"EOF"
        )

        try:
            out = run_cmd(ssh, backup_cmd, timeout=60)
        ...
        # ----------------------------------------------------------
        # STEP 3: Upload ISO + MD5 to /var/sw/ on device
        # ----------------------------------------------------------
        ...
        try:
            expected_md5 = run_cmd(ssh, f"cat /var/sw/{MD5_FILENAME}").split()[0]
            print(f"    Expected : {expected_md5}")

            print(f"    Computing md5sum on device (may take ~60 seconds)...")
            actual_md5 = run_cmd(ssh, f"md5sum /var/sw/{ISO_FILENAME}", timeout=180).split()[0]
        ...
        # All software_upgrade sub-commands sent as one heredoc block
        # Device reboots automatically -- SSH will drop, that is expected
        # ----------------------------------------------------------
        ...
        upgrade_cmd = (
            f"software_upgrade <<EOF\n"
            f"set image_location=local_system\n"
            f"set filename={ISO_FILENAME}\n"
            f"upgrade\n"
            f"EOF"
        )

        # Fire the upgrade command -- we don't wait for a response because
        # the device takes 5-10 min to even start rebooting, so just send it
        try:
            ssh.exec_command(upgrade_cmd, timeout=10)
        except Exception:
            pass
        ...
        #
        # Phase A -- wait for SSH to stop responding (device went down)
        # Phase B -- wait for SSH to start responding again (device is back)
        #
        # We poll ssh.connect() in one continuous loop.
        # No fixed sleep upfront because we don't know exactly when it reboots.
        # ----------------------------------------------------------
        ...
        # ----------------------------------------------------------
        # POST-UPGRADE: Validate -- same ssh object already reconnected above
        # ----------------------------------------------------------
        print(f"\n  [Post] Running post-upgrade validation...")
        try:
            hostname = run_cmd(ssh, "hostname")

            version_out = run_cmd(ssh, "show /settings/system_preferences/")
            version_line = ""
            ...
            uptime_out = run_cmd(ssh, "uptime")

            # Event 103 = software upgrade completed
            events_out = run_cmd(ssh, "show /system/event_list/")
            upgrade_event = "Not found in event log"
            for line in events_out.splitlines():
                if "103" in line and "upgrade" in line.lower():
                    upgrade_event = line.strip()
                    break

            print(f"    Hostname  : {hostname}")
            print(f"    Version   : {version_line}")
            print(f"    Uptime    : {uptime_out}")
            print(f"    Event 103 : {upgrade_event}")
        ...
        # Always runs -- closes the SSH session no matter what happened above
        ssh.close()
...
# Run upgrade on each device one by one
results = []
